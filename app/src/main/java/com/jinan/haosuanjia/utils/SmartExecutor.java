...
public class SmartExecutor implements Executor {
        ...
        synchronized (lock) {
            ...
            if (waitingList.size() > 0) {
                ...
                if (waitingRun != null) {
                    ...
                    LogX.v(TAG, "Thread " + Thread.currentThread().getName() + " execute next task..");
                } else {
                    LogX.e(TAG,
                            "SmartExecutor get a NULL task from waiting queue: " + Thread.currentThread().getName());
                }
            } else {
                    LogX.v(TAG, "SmartExecutor: all tasks is completed. current thread: " +
    ...
    public SmartExecutor setCoreSize(int coreSize) {
        ...
        this.coreSize = coreSize;
            LogX.v(TAG, "SmartExecutor core-queue size: " + coreSize + " - " + queueSize
